...
uint16_t CalculateCRC16(const uint8_t *msg, uint8_t len)
{
    uint8_t crc_hi = 0xFF;
    uint8_t crc_lo = 0xFF;
    uint8_t index;
    ...
    while (len--) {
        index = crc_lo ^ *msg++;
        crc_lo = crc_hi ^ TabCRCHi[index];
        crc_hi = TabCRCLo[index];
    }

    // Return CRC with low byte first (as per Modbus spec)
    return ((uint16_t)crc_lo << 8) | crc_hi;
}

void ancit_esp_reset(void) {
