...
void fota_uart_init(){
	...
	Init_ret=LPUART_DRV_Init(UART_CHANNEL, &lpUartState1, &lpUartInitConfig1);
...
void fota_uart_tx_sm(void){
	...
	case UART_TX_SM_SEND_START:
		...
		LPUARTStatus_tx = LPUART_DRV_SendData(UART_CHANNEL, (const uint8_t*) UartData.tx_data,3);
...
void fota_process_data(void){
	...
	case TYPE_0:
		// OTA initiated acknowledge and wait for the next line of code
